  ...
  try {
    await db.query(
      `INSERT INTO clinic_treatment_durations (
         ...
         started_at, dentist_id, chair_id, source
       )
       SELECT $1, $2, $3, $4, CURRENT_TIMESTAMP, $5, $6, 'queue'
       WHERE NOT EXISTS (
         SELECT 1 FROM clinic_treatment_durations
         WHERE queue_entry_id = $3 AND completed_at IS NULL
       )`,
      [
    ...
    const open = await db.query(
      `SELECT id, started_at
       ...
       WHERE completed_at IS NULL
         AND (
           ($1::bigint IS NOT NULL AND queue_entry_id = $1)
           OR ($2::bigint IS NOT NULL AND appointment_id = $2)
           OR ($3::text IS NOT NULL AND patient_user_id = $3)
         )
       ORDER BY
         CASE WHEN queue_entry_id = $1 THEN 0 ELSE 1 END,
         id DESC
       LIMIT 1`,
      [queueEntryId || null, appointmentId || null, patientUserId ? String(patientUserId) : null]
    );
