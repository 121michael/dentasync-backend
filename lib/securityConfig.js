...
/**
 * Some Windows npm versions set NODE_ENV=production for `npm start`. Combined with a
 * template JWT_SECRET like "your_super_secret_key_here" (≥16 chars but still weak),
 * the process throws during require() and the terminal looks like it "does nothing".
 * Downgrade to development for local npm lifecycle scripts when secrets are weak.
 * Mutates and returns env.
 */
function normalizeLocalNpmStartEnv(env = process.env) {
  const lifecycle = String(env.npm_lifecycle_event || "");
  const isNpmStartScript = lifecycle === "start" || lifecycle === "start:backend";
  if (!isNpmStartScript) {
  ...
  if (env.FORCE_SECURE_SECRETS === "true") {
    return { env, changed: false };
  }
  ...
  const passwordResetSecret =
    env.PASSWORD_RESET_SECRET || env.OTP_SECRET || env.JWT_SECRET;
  const hasStrongSecrets =
    !isWeakSecret(jwtSecret) &&
    !isWeakSecret(otpSecret) &&
    !isWeakSecret(passwordResetSecret);
...
/**
 * Normalize a browser origin string from FRONTEND_URL / CORS_ORIGINS.
 * Accepts full URLs (with path) and strips path/query so Origin matching works.
 */
function normalizeOrigin(value) {
  ...
  try {
    ...
    // Already an origin-like value without a path (e.g. https://clinic.example).
...
function createCorsOptions(env = process.env) {
  ...
  if (production && allowedOrigins.length === 0) {
    throw new Error(
      "FRONTEND_URL or CORS_ORIGINS must list at least one browser origin in production."
    );
  }
